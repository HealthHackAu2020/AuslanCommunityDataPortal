...
mutation UpdateUser($id: ID!, $name: String, $profile: String, $email: String, $password: String)
{
    updateUser(id:$id, data:{ name: $name, profile: $profile, email: $email, password: $password })
    {
        id
        name
    }
...
export default function Profile() {
    ...
    const {data, error, loading, refetch } = useQuery(printGraphql(getAuthenticatedUserQuery));

    const [updateName] = useMutation(printGraphql(updateNameMutation));
    const [deleteProfilePic] = useMutation(printGraphql(deleteProfilePicMutation));
    ...
    const onSubmit = async ({ email, password, name, profile }) => {
        try {
            ...
            // Call auth hook
            updateName(
            { variables: { id: data.authenticatedUser.id, name: name, profile: profile, email: email, password: password } }
            )
        } catch (error) {
        }
    };
    ...
    const onDeleteProfile = async () => {
        try {
            ...
            await deleteProfilePic(
                { variables: { id: data.authenticatedUser.id } }
            )
            await refetch();
        } catch (error) {
        }
    ...
    const onChange = async (e) => {
        ...
        await updateProfilePic( { variables: { id: data.authenticatedUser.id, image: fileObj }});
    ...
    {
        showProfilePic = <>
            <img src={data.authenticatedUser.image.publicUrl} alt='Your Profile Picture' width='160' height='210'/>
